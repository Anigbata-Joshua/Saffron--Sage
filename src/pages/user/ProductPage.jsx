...
const relatedProducts = [
  { img: "./image/4th cart.1.jpg", name: "Tie Waist Maxi Dress in Blue", price: "₦260,000" },
  { img: "./image/od4`.jpg", name: "Fruito Yolk Blouse in Yellow", price: "₦189,800" },
  { img: "./image/product3.jpg", name: "Fruito Bib Smock Dress in Blush", price: "₦189,800" },
  { img: "./image/1_front_etch__15523.jpg", name: "Fruito Bib Smock Dress in Blush", original: "₦316,300", price: "₦189,800" },
];
...
export default function ProductPage() {
      ...
      <div className="bg-white min-h-screen">
        ...
        <div className="flex items-center justify-center h-[60vh]">
          <motion.p animate={{ opacity: [0.4, 1, 0.4] }} transition={{ repeat: Infinity, duration: 1.5 }}
            className="text-gray-400 uppercase tracking-widest text-xs font-medium">Loading product...</motion.p>
        </div>
      </div>
    );
  }
  ...
  if (!product) return (
    <div className="bg-white min-h-screen">
      <Navbar />
      <div className="flex items-center justify-center h-[60vh]">
        <p className="text-gray-400 uppercase tracking-widest text-xs font-medium">Product not found.</p>
      </div>
    </div>
  );

  const imagePath = product.images || product.image || "https://via.placeholder.com/800x1000";
      ...
      <div className="bg-white">
        ...
        <section className="w-[92%] lg:w-[80%] mx-auto px-2 sm:px-4 mt-6 lg:mt-12">
          ...
          >
            ...
            <motion.div variants={slideInLeft} className="grid grid-cols-2 gap-1.5 sm:gap-2">
              <div className="overflow-hidden aspect-[3/4] bg-stone-50">
                <motion.img src={imagePath} alt={product.title} className="w-full h-full object-cover"
                  whileHover={{ scale: 1.02 }} transition={{ duration: 0.3 }} />
              </div>
              <div className="overflow-hidden aspect-[3/4] bg-stone-50">
                <motion.img src={imagePath} alt={`${product.title} Alternate`} className="w-full h-full object-cover"
                  whileHover={{ scale: 1.02 }} transition={{ duration: 0.3 }} />
              </div>
            </motion.div>

            {/* Right Column: Product Core Meta Info Details */}
            <motion.div variants={slideInRight} className="space-y-5 lg:pr-6">
              ...
              <p className="text-lg font-semibold text-stone-950 tracking-wider">₦{Number(String(product.price).replace(/,/g, "")).toLocaleString()}</p>
              ...
              <div className="border-b border-stone-100 pb-2">
                <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}
                  className="px-4 py-2 text-[10px] tracking-widest font-semibold text-stone-700 border border-stone-200 uppercase hover:bg-stone-50 transition-colors">
                  Write a review
                </motion.button>
              </div>

              {/* Sizes Selection Strip */}
              <div className="space-y-3">
                <div className="flex justify-between items-center text-xs uppercase tracking-wider font-semibold text-stone-800">
                  <p>Size: <span className="text-stone-500 font-normal normal-case">{selectedSize !== null ? `Size ${selectedSize}` : "Select dynamic size"}</span></p>
                ...
                <div className="flex flex-wrap gap-2">
                  {[0, 1, 2].map((s) => (
                    <button 
                      ...
                      className={`border w-11 h-11 flex items-center justify-center text-xs font-semibold tracking-widest cursor-pointer transition-all duration-200 ${
                        selectedSize === s 
                          ? "border-black bg-black text-white shadow-sm" 
                          : "border-stone-200 text-stone-700 hover:border-black hover:bg-stone-50"
                      }`}
                    >
                      {s}
                    </button>
              ...
              {/* Interactive Form Submissions Call To Action Elements Area */}
              <div className="pt-4 space-y-3">
                <motion.button
                  onClick={handleAddToCart}
                  whileHover={{ backgroundColor: "#1c1917" }}
                  whileTap={{ scale: 0.99 }}
                ...
                <select className="w-full border border-stone-200 p-3.5 text-xs text-stone-600 bg-white uppercase tracking-widest font-medium rounded-none focus:outline-none focus:border-stone-400 cursor-pointer">
          ...
          {/* Descriptive Information Section Accordion Tabs Component Line */}
          <motion.div className="mt-16 lg:mt-24 w-full lg:w-[65%] mb-16" variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
            ...
            <motion.div
              ...
              initial="hidden"
              animate="visible"
              className="mt-6 text-stone-600 text-xs sm:text-[13px] font-normal leading-relaxed whitespace-pre-line px-1"
            >
              {DETAILS[activeTab]}
            </motion.div>
          </motion.div>

          {/* Related Cross Sell Products Grid Dynamic Section */}
          <div className="border-t border-stone-100 pt-12 lg:pt-16">
            ...
            <motion.div
              className="grid grid-cols-2 md:grid-cols-4 mt-8 lg:mt-12 mb-20 gap-x-3 gap-y-8 sm:gap-6"
              variants={staggerContainer}
              initial="hidden"
              whileInView="visible"
            ...
            >
              {relatedProducts.map((p, i) => (
                <motion.div key={i} variants={staggerItem} whileHover={{ y: -4, transition: { duration: 0.2 } }} className="group">
